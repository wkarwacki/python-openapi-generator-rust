...
impl Context {
    ...
    pub fn of(path: PathBuf) -> Self {
        ...
        map.insert(path.file_name().unwrap().to_str().map(|s| s.to_string()), value.clone());
    ...
    fn get_of(value: Value, base: PathBuf, map: &mut HashMap<Option<String>, Value>) {
        match value  {
            Value::Mapping(ref mapping) => {
                ...
                mapping.iter().for_each(|(_, v)| Self::get_of(v.clone(), base.clone(),  map));
            },
            Value::Sequence(sequence) => sequence.iter().for_each(|value| Self::get_of(value.clone(), base.clone(),  map)),
            _ => ()
        }
    }
    ...
    pub fn resolve<T: de::DeserializeOwned>(&self, r#ref: String) -> T
    {
        ...
        let value = path.split('/')
            ...
            .fold(self.val.get(&src).clone().unwrap(), |acc, val| {
                &acc[val]
            });
        // FIXME_LATER: src should be resolved properly (by not guessing), it requires passing original context src to this fn
        let result = match value {
            Value::Null => {
                let guess = self.val.iter().find(|(_, val)| {
                    let open_api: OpenApi = serde_yaml::from_value(val.clone().clone()).unwrap();
                    open_api.components.schemas.contains_key(path.split("/").last().unwrap())
                }).unwrap().1;
                path.split('/')
                    .filter(|str| !str.is_empty())
                    .fold(guess, |acc, val| {
