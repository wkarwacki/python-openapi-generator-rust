...
impl Lang for LangPython {
    ...
    fn fmt_value(
        ...
        match json_value {
            ...
            Value::String(val) => def
                .as_ref()
                .map(|d| match d {
                    Def::Enum(Enum { vals, null: _ }) => match vals {
                        EnumVals::Int(_) => {
                            self.fmt_type(d, name) + "." + self.fmt_enum(val).as_str()
                        }
                        EnumVals::Str(_) => {
                            self.fmt_type(d, name) + "." + self.fmt_enum(val).as_str()
                        }
            ...
            Value::Array(val) => {
                ...
                "[".to_string()
                    + val
                        .iter()
                        .map(|json_value| {
                            self.fmt_value(json_value, &item, &name.as_deref(), context)
                        })
                        .collect::<Vec<String>>()
                        .join(", ")
                        .as_str()
            ...
            Value::Object(val) => val
                .iter()
                .map(|(key, json_value)| {
                    key.clone() + ": " + self.fmt_value(json_value, &None, &None, context).as_str()
                })
                .collect::<Vec<String>>()
                .join(", "),
